...
class TorchTrainer():
    ...
    def fit(self, dl_train: DataLoader, dl_val: DataLoader,
        ...
        #our own addition to the to the code: ----
        best_train_acc = -1
        #------
        for epoch in range(num_epochs):
            ...
            # ====== YOUR CODE: ======
            epoch_loss, epoch_acc = self.train_epoch(dl_train)
            epoch_val_loss, epoch_val_acc = self.test_epoch(dl_test=dl_val)
            ...
            if(early_stopping is not None and epoch > 0):
                if(epoch_acc > best_train_acc):
                    ...
                    stopping_counter+=1
                    if (early_stopping == stopping_counter):
                        print("Early Stop Intiallized")
                        break
            # ========================
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        loss.backward()
        self.optimizer.step()
        out_torch = torch.argmax(output,1)
        if(self.device):
            out_torch.to(self.device)
        num_correct = (torch.sum(out_torch == y)).item()
        loss = loss.item()
        # ========================
