...
class ConvClassifier(nn.Module):
    ...
    def _make_feature_extractor(self):
        ...
        for i in range(len(self.filters)):
          layers+=[nn.Conv2d(prev_channel, self.filters[i], kernel_size=3, padding =1),nn.ReLU()]
...
class YourCodeNet(ConvClassifier):
    ...
    def _make_feature_extractor(self):
        ...
        prev_channel = in_channels
        for i in range(len(self.filters)):
          conv = nn.Conv2d(prev_channel, self.filters[i], kernel_size=3, padding=1)
          if i == 0:
             nn.init.xavier_normal(conv.weight)
             nn.init.zeros_(conv.bias)
          layers+=[conv, nn.BatchNorm2d(self.filters[i], device=device), #Q1+Q2
          nn.ELU()]
          prev_channel = self.filters[i]
          if (self.pool_every <= len(self.filters)) and ((i + 1) % self.pool_every == 0):
                layers.append(nn.MaxPool2d(kernel_size = 2, stride= 2))
    ...
    def _make_classifier(self):
        ...
        for i in range(len(self.hidden_dims)):
            layers += [nn.Linear(prev_layer, self.hidden_dims[i]), nn.BatchNorm1d(self.hidden_dims[i]), #Q1+Q2
             nn.ELU()]
            prev_layer = self.hidden_dims[i]
        layers.append(nn.Linear(prev_layer, self.out_classes))
        seq = nn.Sequential(*layers)
